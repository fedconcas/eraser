...
func (s *Server) handleAPIInboxScan(w http.ResponseWriter, r *http.Request) {
	...
	// Delivery-failure notices generated by our own mail system (not the
	// broker's) can never be attributed by sender domain - see
	// attributeBounceRepliesByRecipient - so check for them separately.
	if bounceReplies, err := s.attributeBounceRepliesByRecipient(ctx, monitor, emails); err != nil {
		log.Printf("Warning: failed to check for delivery-failure notices: %v", err)
	} else if len(bounceReplies) > 0 {
		emails = append(emails, bounceReplies...)
	}
