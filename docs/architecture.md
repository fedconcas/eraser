...
│       │                        # by resource: welcome.go (the /welcome landing tab),
│       │                        # bounces.go (self-healing broker list), handlers_pages.go
│       │                        # (dashboard/brokers/history/
...
- `tags`: Optional list of *disposition* tags from the closed vocabulary in `broker.DispositionTags` - `b2b-only` (told us it holds no consumer data at all), `form-only` (holds consumer data but refuses email) and `us-data-only` (told us it only holds data on US customers). Distinct from `category`, which is the sector; see [code-patterns.md](code-patterns.md) for why. **`us-data-only` is a scope note, not a refusal**: the company holds consumer data and answers requests, just not for people outside the US, so it never blocks a send - most users of this tool are in the US, where such a broker is exactly the one they need to write to. `Broker.Sendable()` blocks every send path to a broker tagged `b2b-only` or `form-only`, and both the web UI's disposition selector and `list-brokers --tag` filter on them. Tags are applied with `eraser tag-broker <id> <tag>` or the per-row Tag select on the web UI's brokers page (which defaults to showing only sendable brokers; non-sendable ones stay in the data file and surface via "Include non-sendable"). `eraser audit-brokers` supplies the evidence for tagging: website/MX liveness checks that flag defunct brokers, plus `--replies` exports of stored broker replies for offline classification.
